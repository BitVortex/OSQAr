...
class TestStateMachine:
    ...
    def test_hysteresis_deadband(self):
        ...
        assert state == TemperatureState.SAFE, "Must transition to SAFE at ≤95°C"
...
class TestConfiguration:
    ...
    def test_hysteresis_constraint(self):
        ...
        # Valid configuration
        config = TemperatureConfig(
            threshold_high_celsius=100.0, threshold_low_celsius=95.0
        )
        assert config is not None

        # Invalid: low >= high should raise error
        with pytest.raises(ValueError):
            TemperatureConfig(threshold_high_celsius=100.0, threshold_low_celsius=100.0)

        with pytest.raises(ValueError):
            TemperatureConfig(threshold_high_celsius=100.0, threshold_low_celsius=105.0)
