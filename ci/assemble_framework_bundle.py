    ...
    (bundle_root / "BUNDLE_README.txt").write_text(
        "\n".join(
            [
                ...
                "- tools/: stdlib-only CLI tooling (python -m tools.osqar_cli)",
                "- templates/: project scaffolding templates used by the CLI",
                "- osqar / osqar.cmd / osqar.ps1: convenience wrappers for the CLI",
                "",
                "Quickstart:",
                "- With Poetry (recommended): poetry run python -m tools.osqar_cli --help",
                "- Without Poetry: python -m tools.osqar_cli --help (uses your active Python env)",
                "",
                "Notes:",
                "- Example shipments are published as separate release assets.",
                "- To run example *.sh scripts unchanged on Windows, prefer WSL2.",
                "",
            ]
        )
        + "\n",
