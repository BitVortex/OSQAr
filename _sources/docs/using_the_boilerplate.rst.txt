...
Projects that author ``STDCLAIM_*`` needs must also identify the project catalog
declarations::
...
catalog, for example::

   {
     "standards": {
       "catalogs": [
         {"id": "project-standard", "source": "standards/catalog.json"}
       ]
     }
   }

Then replace each example claim's ``standards_catalog``, ``standards_refs``,
``project_interpretation``, and ``applicability`` values. Remove example claims
that the project does not adopt. A user catalog's minimal machine-readable
contract is a JSON object with an ``entries`` array; every entry needs a unique,
non-empty ``reference_id``::

   {
     "entries": [
       {"reference_id": "PROJECT-STANDARD-REF-001"}
     ]
   }

Catalog paths are resolved relative to ``osqar_project.json``. Keep the catalog
under project control and use stable identifiers; the minimal contract proves
only that an authored identifier resolves, not that its interpretation is
correct.

Standards-claim links are authored from the claim to project objects:

* ``realized_by`` points to requirements or lifecycle records (``REQ_*`` or
  ``LM_*``) that implement or instantiate the project interpretation;
* ``verified_by`` points to planned or executed verification activities
  (``VER_*``) that evaluate it; and
* ``evidenced_by`` points to evidence records (``EVID_*``) associated with that
  evaluation.

A reverse link generated by Sphinx is navigation, not a substitute for one of
these authored claim-to-target links. A resolvable ``evidenced_by`` link also
does not mean that the artifact exists, an activity ran, a result passed, or a
reviewer accepted the evidence. Keep placeholder evidence explicitly pending
until execution, provenance, result, and review state are recorded.

``shipment traceability --project-config`` propagates the same catalog check.
``shipment prepare`` and ``doctor`` use ``<project>/osqar_project.json``
automatically when it exists. Projects without ``STDCLAIM_*`` needs remain
valid without a project configuration.
